...
def config_for(
    *,
    ...
    config = {
        ...
        "problem": problem,
        "output_dir": (
            f"{budget_cfg['output_prefix']}/npe_{imputation}_imputation/"
            ...
            for problem in PROBLEMS:
                for mechanism in MECHANISMS:
                    for eps in EPSILONS:
                        path = (
                            ROOT
                            / f"{imputation}_imputation"
                            / budget
                            / problem
                            / (
                                f"{problem}_{imputation}_{mechanism}_"
                                f"eps{eps}_config.yaml"
                            )
                        )
                        write_config(
                            path,
                        ...
                        written += 1
    print(f"Wrote {written} NPE imputation configs under {ROOT}")

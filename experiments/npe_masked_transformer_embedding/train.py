...
from experiments.npe_mask_augmentation.train import (
...
def _embedding_config(
    ...
    return MaskedTransformerEmbeddingConfig(
        x_dim=int(x_dim),
...
def main() -> None:
    ...
    method_name = "npe_masked_transformer_embedding"
    ...
    for seed in seeds:
        ...
        embedding_net = make_masked_transformer_embedding(embedding_config)
        ...
        summary = {
            ...
            "embedding_num_parameters": int(embedding_num_parameters),
            "embedding_type": "masked_transformer",
        ...
        if reference_path:
            ...
            if "mask" not in metadata or not isinstance(metadata["mask"], dict):
                raise ValueError(
                    ...
                    "reference observations under masked transformer embedding."
                )
            ...
            x_ref = make_zero_imputed_mask_condition(
            ...
            reference_sampling_time_sec = (
                time.perf_counter() - reference_sampling_start
            )
            ...
            with h5py.File(reference_posterior_samples_path, "w") as f:
                ...
                f.attrs["reference_metric_target"] = (
                    "masked_transformer_x_vs_full_x_reference"
                )
            ...
            summary.update(
                {
                    ...
                    "reference_metric_target": "masked_transformer_x_vs_full_x_reference",

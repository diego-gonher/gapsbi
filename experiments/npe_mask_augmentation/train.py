...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train NPE Wang-style mask-augmentation baseline on x_obs."
    )
    parser.add_argument(
        "--config",
        type=Path,
...
def main() -> None:
    ...
    x_obs_train_scaled, x_obs_val_scaled, x_obs_test_scaled, _x_scaler, x_scaling_metadata = (
        scale_x_obs_train_val_test_from_full_train(
    ...
    x_train = make_zero_imputed_mask_augmented_input(
        x_obs_scaled=x_obs_train_scaled,
        mask=mask_train,
    )
    x_val = make_zero_imputed_mask_augmented_input(
        x_obs_scaled=x_obs_val_scaled,
        mask=mask_val,
    )
    x_test = make_zero_imputed_mask_augmented_input(
    ...
    for seed in seeds:
        ...
        summary = {
            ...
            "x_dim": int(x_dim),
            "x_dim_original": int(x_dim_original),

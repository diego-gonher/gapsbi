...
def config_for(*, problem: str, budget: str, mechanism: str, eps: str) -> dict:
    ...
    config = {
        ...
        "problem": problem,
        "output_dir": (
            f"{budget_cfg['output_prefix']}/npe_mask_augmentation/"
            ...
            for mechanism in MECHANISMS:
                for eps in EPSILONS:
                    path = (
                        ...
                        / budget
                        / problem
                        / f"{problem}_npe_mask_augmentation_{mechanism}_eps{eps}_config.yaml"
                    )
                    write_config(
                        path,
                        config_for(
                    ...
                    written += 1
    print(f"Wrote {written} NPE mask-augmentation configs under {ROOT}")

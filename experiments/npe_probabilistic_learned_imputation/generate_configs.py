...
def config_for(*, problem: str, budget: str, mechanism: str, eps: str) -> dict:
    ...
    lambda_np = 1.0 if mechanism == "mnar" else 100.0
    config = {
        "dataset_path": DATASETS[(problem, mechanism)].format(eps=eps),
        "problem": problem,
        "output_dir": (
            f"{budget_cfg['output_prefix']}/"
        ...
        "rise": {
            ...
            "lambda_mask": 1.0,
            "learning_rate": 1e-3,
            "batch_size": budget_cfg["training_batch_size"],
            "max_num_epochs": 5_000,
            "stop_after_epochs": 20,
            "imputer_type": "mlp",
            "hidden_dim": 128,
...
def main() -> None:
            ...
            for mechanism in MECHANISMS:
                for eps in EPSILONS:
                    path = (
                        ...
                        / problem
                        / (
                            f"{problem}_npe_probabilistic_learned_imputation_"
    ...
    print(
        "Wrote "
        f"{written} NPE probabilistic learned-imputation configs under {ROOT}"
    )
